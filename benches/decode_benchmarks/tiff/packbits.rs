//! Criterion benchmark for PackBits-compressed TIFF decode (compression tag 32773).
//!
//! Feature-gated on `libtiff`. Setup writes a synthetic 2048×2048×3 tiled TIFF
//! with PackBits compression, reads it back, and benchmarks `get_block(0, 0, 0, None)`.
...
/// Benchmark PackBits-compressed TIFF single-tile decode.
pub fn bench_tiff_packbits(c: &mut Criterion) {
    ...
    // Compression=32773 (PackBits), tile 2048×2048
    let metadata = BufferedMetadataProvider::new();
    metadata.set_json("259", serde_json::json!(32773));
    ...
    // --- Benchmark ---
    let mut group = c.benchmark_group("tiff_packbits");
