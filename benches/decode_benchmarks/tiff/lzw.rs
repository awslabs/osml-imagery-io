//! Criterion benchmark for LZW-compressed TIFF decode (compression tag 5).
//!
//! Feature-gated on `libtiff`. Setup writes a synthetic 2048×2048×3 tiled TIFF
//! with LZW compression, reads it back, and benchmarks `get_block(0, 0, 0, None)`.
...
/// Benchmark LZW-compressed TIFF single-tile decode.
pub fn bench_tiff_lzw(c: &mut Criterion) {
    ...
    // Compression=5 (LZW), tile 2048×2048
    let metadata = BufferedMetadataProvider::new();
    metadata.set_json("259", serde_json::json!(5));
