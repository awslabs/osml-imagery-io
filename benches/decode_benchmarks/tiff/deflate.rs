//! Criterion benchmark for Deflate-compressed TIFF decode (compression tag 8).
//!
//! Feature-gated on `libtiff`. Setup writes a synthetic 2048×2048×3 tiled TIFF
//! with Deflate compression, reads it back, and benchmarks `get_block(0, 0, 0, None)`.
...
/// Benchmark Deflate-compressed TIFF single-tile decode.
pub fn bench_tiff_deflate(c: &mut Criterion) {
    ...
    // Compression=8 (Deflate), tile 2048×2048
    let metadata = BufferedMetadataProvider::new();
    metadata.set_json("259", serde_json::json!(8));
    ...
    // --- Benchmark ---
    let mut group = c.benchmark_group("tiff_deflate");
