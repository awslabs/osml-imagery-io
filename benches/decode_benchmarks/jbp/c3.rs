//! Criterion benchmark for JPEG DCT (IC=C3) decode via the JBP writer/reader pipeline.
//!
//! Feature-gated on `libjpeg-turbo`. Setup writes a synthetic 2048×2048×3 image as
//! a NITF with IC=C3, reads it back, and benchmarks `get_block(0, 0, 0, None)`.
...
/// Benchmark JPEG DCT (IC=C3) single-block decode.
pub fn bench_jbp_c3(c: &mut Criterion) {
    ...
    // 2. Create provider and fill with deterministic synthetic data (BSQ format)
    ...
    // --- Benchmark ---
    let mut group = c.benchmark_group("jbp_c3");
