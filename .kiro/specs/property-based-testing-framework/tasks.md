...
- [x] 1. Set up test infrastructure and shared utilities
  - [x] 1.1 Create tests/property/ directory structure with __init__.py
    - Create tests/property/__init__.py
    - _Requirements: 9.1_
  
  - [x] 1.2 Create conftest.py with shared fixtures and pytest configuration
  ...
  - [x] 1.3 Create quality.py with PSNR and SSIM calculation functions
...
- [x] 2. Implement image generation strategies
  - [x] 2.1 Create strategies.py with core strategy functions
  ...
  - [x] 2.2 Implement image_arrays() and random_image() composite strategies
  ...
  - [x] 2.3 Implement edge_case_images() strategy
  ...
  - [x] 2.4 Implement valid_block_coordinates() strategy
...
- [x] 3. Checkpoint - Verify strategies work correctly
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement roundtrip property tests
  - [x] 4.1 Create test_roundtrip.py with test class structure
  ...
  - [x] 4.2 Write property test for lossless roundtrip
  ...
  - [x] 4.3 Write property test for lossy roundtrip with quality bounds
    ...
    - **BUG DISCOVERED & FIXED**: Test discovered byte order mismatch bug in J2K decoder. Fixed by changing `to_be_bytes()` to `to_ne_bytes()` in `src/jbp/j2k/openjpeg.rs` for both `decode()` and `decode_tile()` methods.
  
  - [x] 4.4 Write property tests for idempotent encoding
    - **Property 10: Idempotent Encoding (Byte-Level)**
    - **Property 11: Idempotent Encoding (Value-Level)**
    - **Validates: Requirements 6.1, 6.2**

- [x] 5. Implement block access property tests
  - [x] 5.1 Create test_block_access.py with test class structure
  ...
  - [x] 5.2 Write property test for block access completeness
  ...
  - [x] 5.3 Write property test for block reassembly roundtrip
  ...
  - [x] 5.4 Write property test for invalid block coordinate error handling
  ...
  - [x] 5.5 Write property test for resolution level consistency
...
- [x] 6. Implement metadata property tests
  - [x] 6.1 Create test_metadata.py with test class structure
    - Create TestMetadataRoundtrip class
    - _Requirements: 5.1_
  
  - [x] 6.2 Write property test for metadata roundtrip preservation
...
- [x] 8. Migrate existing property tests to unified framework
  - [x] 8.1 Migrate API design alignment property tests
    - Move tests from tests/test_api_design_alignment_pbt.py to tests/property/test_api_contracts.py
    - Create TestAssetProviderPolymorphism class with @pytest.mark.property decorator
    - Migrate test_add_asset_accepts_bytes_asset_provider (uses @given)
    - Migrate test_add_asset_accepts_buffered_image_asset_provider (uses @given)
    - Update imports to use shared strategies where applicable
    - Delete original tests/test_api_design_alignment_pbt.py after migration
    - _Requirements: 9.1, 9.5_
  
  - [x] 8.2 Migrate JBP IO format auto-detection tests
    - Move Property 23 tests from tests/test_jbp_io.py to tests/property/test_io_contracts.py
    - Create TestFormatAutoDetection class with @pytest.mark.property decorator
    - Convert example-based tests to property tests where beneficial (e.g., test_has_asset_consistency)
    - Keep tests that are inherently example-based but organize under property module
    - _Requirements: 9.1, 9.5_
  
  - [x] 8.3 Migrate JBP IO round-trip consistency tests
    - Move Property 20 tests from tests/test_jbp_io.py to tests/property/test_io_contracts.py
    - Create TestDatasetRoundTripConsistency class with @pytest.mark.property decorator
    - These are regression tests with fixed files - keep as example-based but organized with property tests
    - _Requirements: 9.1, 9.5_
  
  - [x] 8.4 Clean up original test files
    - Delete tests/test_jbp_io.py after all tests migrated
    - Verify no broken imports or references
    - Run full test suite to confirm migration successful
    - _Requirements: 9.1_

- [x] 9. Create documentation
  - [x] 9.1 Create docs/PROPERTY_BASED_TESTING.md
  ...
  - [x] 9.2 Update README.md Testing section
  ...
  - [x] 9.3 Update .kiro/steering/tech.md with property testing commands
  ...
  - [x] 9.4 Update .kiro/steering/structure.md with test organization
...
- [x] 10. Final checkpoint - Verify complete framework
  - Ensure all tests pass, ask the user if questions arise.
