...
- [ ] 1. Update IO.open to accept list of paths
  - [ ] 1.1 Modify IO.open signature in src/bindings/io.rs
  ...
  - [ ] 1.3 Write unit tests for IO.open edge cases
    - Test single-element list
    - Test multi-element list (verify first is used)
    - Test empty list raises ValueError
    - _Requirements: 1.2, 1.3_

- [ ] 2. Update DatasetReader to use metadata property
  - [ ] 2.1 Change get_metadata() to metadata property in src/bindings/reader.rs
...
- [ ] 3. Update DatasetWriter API
  - [ ] 3.1 Remove add_image_asset method from src/bindings/writer.rs
    - Delete the `add_image_asset` method entirely
    - Existing `add_asset` already accepts Arc<dyn AssetProvider>
    - _Requirements: 3.1_
  
  - [ ] 3.2 Convert set_metadata to metadata property setter in src/bindings/writer.rs
  ...
  - [ ] 3.3 Write property test for add_asset type hierarchy
    - **Property 3: add_asset accepts all AssetProvider subtypes**
    - **Validates: Requirements 3.2**

- [ ] 4. Checkpoint - Verify core API changes
  - Ensure all tests pass, ask the user if questions arise.
  - Run `cargo test` and `pytest tests/`

- [ ] 5. Update TextAssetProvider to use text property
  - [ ] 5.1 Change get_text() to text property in src/bindings/text.rs
    - Add `#[getter]` attribute to the method
...
- [ ] 6. Update DataAssetProvider API
  - [ ] 6.1 Change get_mime_type() to mime_type property in src/bindings/data.rs
    - Add `#[getter]` attribute to the method
    - Rename method from `get_mime_type` to `mime_type`
    - _Requirements: 7.2_
  
  - [ ] 6.2 Update parse_as_xml to return ElementTree in src/bindings/data.rs
...
- [ ] 7. Checkpoint - Verify all binding changes
  - Ensure all tests pass, ask the user if questions arise.
  - Run `cargo test` and `pytest tests/`

- [ ] 8. Update API_DESIGN.md - Core API Structure
  - [ ] 8.1 Update IO class diagram
    - Remove fsspec filesystem parameter
    - Change signature to `open(paths: List[str], mode: str, format: Optional[str])`
    - _Requirements: 1.1, 1.4, 1.5_
  
  - [ ] 8.2 Update DatasetReader class diagram
    - Replace `get_metadata()` with `metadata` property
    - _Requirements: 2.1_
  
  - [ ] 8.3 Update DatasetWriter class diagram
    - Replace `set_metadata()` with `metadata` property setter
    - Document that add_asset accepts any AssetProvider
    - _Requirements: 3.3, 3.4_

- [ ] 9. Update API_DESIGN.md - Asset Provider Hierarchy
  - [ ] 9.1 Update AssetProvider class diagram
  ...
  - [ ] 9.2 Update ImageAssetProvider class diagram
    - Show bands parameter as optional in get_block()
    - _Requirements: 5.1_
  
  - [ ] 9.3 Update TextAssetProvider class diagram
    - Replace get_text(), get_encoding(), get_format() with properties
    - _Requirements: 6.1, 6.2, 6.3_
  
  - [ ] 9.4 Update DataAssetProvider class diagram
    - Replace get_mime_type() with mime_type property
    - _Requirements: 7.1_

- [ ] 10. Update API_DESIGN.md - Remove deprecated concepts
  - [ ] 10.1 Remove FilesDatasetReader and FilesDatasetWriter
    - Remove from Format-Specific Implementations diagram
    - Remove any references in text
    - _Requirements: 9.3, 9.4_
  
  - [ ] 10.2 Remove all fsspec mentions
    - Remove filesystem parameter from JBPDatasetReader/Writer
    - Remove any fsspec imports or references
    - _Requirements: 9.5_

- [ ] 11. Update API_DESIGN.md - Add new documentation
  - [ ] 11.1 Update MemoryImageAssetProvider documentation
  ...
  - [ ] 11.2 Add SimpleMetadataProvider section
  ...
  - [ ] 11.3 Add Parser Infrastructure section
