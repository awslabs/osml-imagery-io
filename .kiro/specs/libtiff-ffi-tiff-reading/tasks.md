...
- [x] 1. Build system and feature flag setup
  - [x] 1.1 Add `libtiff` feature flag to `Cargo.toml`
    - Add `libtiff = []` to `[features]` section
    - Add `"libtiff"` to the `default` feature list
    - _Requirements: 7.5, 13.1_

  - [x] 1.2 Add `configure_libtiff()` to `build.rs`
  ...
  - [x] 1.3 Create `src/tiff/mod.rs` with feature gate and empty submodules
...
- [x] 2. Checkpoint - Verify build compiles with libtiff feature
  - Ensure `cargo test` passes with the new feature flag and empty module stubs, ask the user if questions arise.

- [x] 3. Raw FFI declarations and TIFF tag constants
  - [x] 3.1 Implement `src/tiff/sys.rs` with extern "C" declarations
  ...
  - [x] 3.2 Implement `src/tiff/tags.rs` with TIFF tag and format constants
...
- [x] 4. Safe RAII wrapper (TiffHandle)
  - [x] 4.1 Implement `MemoryReadStreamData` and memory callbacks in `src/tiff/ffi.rs`
  ...
  - [x] 4.2 Implement `TiffHandle` struct with `from_bytes()`, `Drop`, and error capture
  ...
  - [x] 4.3 Implement typed tag getters and IFD navigation on `TiffHandle`
  ...
  - [x] 4.4 Implement tile and strip I/O methods on `TiffHandle`
  ...
  - [x] 4.5 Write Rust unit tests for `TiffHandle`
...
- [x] 5. Checkpoint - Verify TiffHandle compiles and links against libtiff
  - Ensure `cargo test` passes with the FFI wrapper, ask the user if questions arise.

- [x] 6. Metadata provider
  - [x] 6.1 Implement `src/tiff/metadata.rs` with `TIFFMetadataProvider`
  ...
  - [x] 6.2 Write Rust unit tests for `TIFFMetadataProvider`
...
- [x] 7. Image asset provider
  - [x] 7.1 Implement pixel format mapping in `src/tiff/image.rs`
  ...
  - [x] 7.2 Implement `TIFFImageAssetProvider` struct and `ImageAssetProvider` trait
  ...
  - [x] 7.3 Implement `get_block()` for tiled TIFFs with chunky-to-BSQ conversion
  ...
  - [x] 7.4 Implement `get_block()` for stripped TIFFs
  ...
  - [x] 7.5 Write Rust unit tests for `TIFFImageAssetProvider`
...
- [x] 8. Checkpoint - Verify image provider compiles and basic tile/strip reading works
  - Ensure `cargo test` passes with the image asset provider, ask the user if questions arise.

- [x] 9. Dataset reader
  - [x] 9.1 Implement `TIFFDatasetReader` in `src/tiff/reader.rs`
  ...
  - [x] 9.2 Implement `DatasetReader` trait for `TIFFDatasetReader`
  ...
  - [x] 9.3 Handle unsupported compression detection
...
- [x] 10. IO factory registration and Python bindings
  - [x] 10.1 Register TIFF format in `src/bindings/io.rs`
  ...
  - [x] 10.2 Write Rust unit tests for IO factory TIFF detection
...
- [x] 11. Checkpoint - Verify end-to-end TIFF reading through IO factory
  - Ensure `cargo test` passes with full TIFF reading pipeline, ask the user if questions arise.

- [x] 12. Python tests
  - [x] 12.1 Write Python unit tests in `tests/test_tiff_reader.py`
...
- [x] 13. Property-based tests
  - [x] 13.1 Add TIFF hypothesis strategies to `tests/property/strategies.py`
  ...
  - [x] 13.2 Write property test for pixel data roundtrip in `tests/property/test_tiff_roundtrip.py`
  ...
  - [x] 13.3 Write property test for band subsetting in `tests/property/test_tiff_roundtrip.py`
  ...
  - [x] 13.4 Write property test for stripped TIFF block dimensions in `tests/property/test_tiff_roundtrip.py`
  ...
  - [x] 13.5 Write property test for block coordinate validation in `tests/property/test_tiff_api.py`
  ...
  - [x] 13.6 Write property test for IFD enumeration in `tests/property/test_tiff_api.py`
  ...
  - [x] 13.7 Write property test for non-image asset access in `tests/property/test_tiff_api.py`
  ...
  - [x] 13.8 Write property test for dataset-level metadata in `tests/property/test_tiff_api.py`
  ...
  - [x] 13.9 Write property test for per-IFD metadata completeness in `tests/property/test_tiff_api.py`
  ...
  - [x] 13.12 Update `tests/property/test_io_contracts.py` for TIFF format detection
