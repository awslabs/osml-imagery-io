...
- [ ] 1. Implement ImageDataMask struct and parsing
  - [ ] 1.1 Create `src/jbp/image/mask.rs` with ImageDataMask struct
  ...
  - [ ] 1.2 Implement ImageDataMask::parse() method
  ...
  - [ ] 1.5 Implement ImageDataMask::from_provided_blocks() constructor
...
- [ ] 2. Implement IC field classification helpers
  - [ ] 2.1 Add IC classification functions to `src/jbp/image/mod.rs`
...
- [ ] 3. Checkpoint - Ensure mask parsing tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Update JBPImageAssetProvider for masked image reading
  - [ ] 4.1 Add mask field to JBPImageAssetProvider
    - Add `mask: Option<ImageDataMask>` field
    - Parse mask during construction when IC is masked
    - _Requirements: 1.1_
  
  - [ ] 4.2 Update has_block() implementation
    - For non-masked images: check bounds only (existing behavior)
    - For masked images: check mask.has_block()
    - _Requirements: 2.1, 2.2_
  
  - [ ] 4.3 Update get_block() implementation
  ...
  - [ ] 4.4 Update pad_pixel_value() implementation
...
- [ ] 5. Update BlockDecoder trait for offset-based decoding
  - [ ] 5.1 Add decode_block_at_offset() to BlockDecoder trait
    - New method signature for decoding at specific offset
    - _Requirements: 2.4_
  
  - [ ] 5.2 Implement decode_block_at_offset() in UncompressedBlockDecoder
    - Seek to offset, read block data, decode
    - _Requirements: 2.4_
  
  - [ ] 5.3 Implement decode_block_at_offset() in J2KBlockDecoder
...
- [ ] 7. Update BufferedImageAssetProvider for sparse block tracking
  - [ ] 7.1 Add provided_blocks field to BufferedImageAssetProvider
    - Add `provided_blocks: HashSet<(u32, u32)>` field
    - _Requirements: 5.1_
  
  - [ ] 7.2 Update set_block() to track provided blocks
    - Insert (block_row, block_col) into provided_blocks
    - _Requirements: 5.1_
  
  - [ ] 7.3 Update has_block() to check provided_blocks
    - Return true only if block is in provided_blocks
    - _Requirements: 5.1_

- [ ] 8. Update JBPDatasetWriter for masked image writing
  - [ ] 8.1 Add collect_provided_blocks() helper method
    - Iterate over block grid, check has_block() for each
    - Return HashSet of provided block coordinates
    - _Requirements: 5.1_
  
  - [ ] 8.2 Add validation for non-masked IC with missing blocks
    - Check if IC is non-masked and blocks are missing
    - Raise MissingBlocks error with expected/provided counts
    - _Requirements: 7.2, 7.3_
  
  - [ ] 8.3 Implement mask generation in write_image_segment()
...
- [ ] 9. Checkpoint - Ensure writer tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Add error types for masking
  - [ ] 10.1 Add BlockNotFound error variant to CodecError
    - Include row and col fields
    - _Requirements: 2.3_
  
  - [ ] 10.2 Add MissingBlocks error variant to CodecError
    - Include expected, provided, and ic fields
    - _Requirements: 7.2, 7.3_
  
  - [ ] 10.3 Add InvalidMaskTable error variant to CodecError
    - Include reason field
    - _Requirements: 1.1_

- [ ] 11. Implement masked image roundtrip property tests
  - [ ] 11.1 Add mask_patterns strategy to tests/property/strategies.py
    - Generate various mask patterns (all_present, all_masked, checkerboard, border_only, random, single_block)
    - _Requirements: 10.1_
  
  - [ ] 11.2 Add masked_image strategy to tests/property/strategies.py
    - Combine image generation with mask pattern generation
    - Include IC value selection (NM, M8)
    - _Requirements: 10.1_
  
  - [ ] 11.3 Create tests/property/test_masking.py
